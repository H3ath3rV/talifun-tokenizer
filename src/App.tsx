...
const disabledSocialLinkClass = "relative overflow-hidden flex items-center gap-2 px-3 py-2 rounded-full text-[13px] font-medium text-gray-500 dark:text-gray-500 hover:text-gray-500 dark:hover:text-gray-500 hover:bg-black/5 dark:hover:bg-white/5 whitespace-nowrap grayscale opacity-75";
const disabledMobileSocialLinkClass = "relative overflow-hidden flex items-center gap-2.5 px-3 py-2.5 rounded-full text-[14px] font-medium text-gray-500 dark:text-gray-500 hover:text-gray-500 dark:hover:text-gray-500 hover:bg-black/5 dark:hover:bg-white/5 grayscale opacity-75";
const disabledLinkOverlayClass = "pointer-events-none absolute inset-0 rounded-full bg-gradient-to-r from-white/70 via-white/25 to-transparent dark:from-black/55 dark:via-black/20 dark:to-transparent";
...
export default function App() {
  ...
  const videoFrameRef = useRef<FullscreenElement>(null);
  const mobileMenuRef = useRef<HTMLElement>(null);
  const menuToggleRef = useRef<HTMLButtonElement>(null);
  const firstMenuLinkRef = useRef<HTMLAnchorElement>(null);
  const lastMenuLinkRef = useRef<HTMLAnchorElement>(null);

  const playVideo = (fullscreen = false) => {
    ...
    const requestFullscreen = frame.requestFullscreen?.bind(frame) ??
      ...
      frame.msRequestFullscreen?.bind(frame);
        ...
        <header className="flex justify-between items-center mb-12 sm:mb-16 relative z-50">
          ...
          <div className="flex items-center gap-3">
            ...
            <nav className="hidden xl:flex items-center gap-0.5 bg-white dark:bg-[#121212]/60 backdrop-blur-xl border border-gray-200/80 dark:border-white/10 rounded-full px-1 py-1 shadow-[0_8px_32px_rgba(0,0,0,0.08)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.12)] mr-2" aria-label="Social links">
              <a href="https://huggingface.co/spaces/talifun/tokenizer-leaderboard" target="_blank" rel="noopener noreferrer" aria-label="Hugging Face (opens in new tab)" className={socialLinkClass}>
          ...
          {isMobileMenuOpen && (
            <nav ref={mobileMenuRef} id="mobile-nav" role="menu" className="absolute top-full left-0 right-0 mt-2 p-1.5 bg-white/90 dark:bg-[#121212]/90 backdrop-blur-2xl border border-white/40 dark:border-white/10 rounded-2xl shadow-xl flex flex-col gap-0.5 xl:hidden max-h-[calc(100dvh-80px)] overflow-y-auto" aria-label="Mobile navigation">
              <a ref={firstMenuLinkRef} href="https://huggingface.co/spaces/talifun/tokenizer-leaderboard" target="_blank" rel="noopener noreferrer" role="menuitem" aria-label="Hugging Face (opens in new tab)" className={mobileSocialLinkClass}>
                <HuggingFaceIcon size={20} />
                Hugging Face
              </a>
              <a href="https://arxiv.org" target="_blank" rel="noopener noreferrer" role="menuitem" aria-label="ArXiv (opens in new tab)" className={mobileSocialLinkClass}>
                <FileText size={20} />
                ArXiv
              </a>
              ...
              <a href="https://reddit.com" target="_blank" rel="noopener noreferrer" role="menuitem" aria-label="Reddit (opens in new tab)" className={mobileSocialLinkClass}>
                <RedditIcon size={20} />
                Reddit
              </a>
              <a ref={lastMenuLinkRef} href="https://linkedin.com" target="_blank" rel="noopener noreferrer" role="menuitem" aria-label="LinkedIn (opens in new tab)" className={mobileSocialLinkClass}>
          ...
          <AnimatedSection className="space-y-10 sm:space-y-12" delay={0}>
            ...
            <AnimatedSection className="my-12 sm:my-16 flex flex-col items-center text-center" delay={0.6} useParentTrigger={true}>
              <h1 className="text-[clamp(40px,6vw,80px)] font-extrabold leading-[1.02] tracking-[-0.02em] max-w-5xl">
                <span className="block bg-gradient-to-r from-[#FF5100] via-[#ff8800] to-[#FF5100] bg-[length:200%_auto] animate-shimmer bg-clip-text text-transparent drop-shadow-none dark:drop-shadow-[0_0_16px_rgba(255,81,0,0.3)] pb-[0.2em] -mb-[0.2em]">Lowest Latency</span>
                <span className="block text-[#171717] dark:text-white">Highest Throughput</span>
              </h1>
            </AnimatedSection>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                ...
                <AnimatedSection className="col-span-full mt-1 sm:mt-3 text-center" delay={1.75} useParentTrigger={true}>
                  <p className="text-[12px] sm:text-[13px] text-gray-500 dark:text-gray-400 tracking-[0.015em] opacity-90">
                    Note: Each bar shows the best published end-to-end o200k throughput for that implementation.
                  </p>
                </AnimatedSection>
              </div>
            </div>
          ...
          <AnimatedSection className="space-y-6 sm:space-y-8 mt-10 sm:mt-12" delay={0}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              ...
              <AnimatedSection className="col-span-full mt-1 sm:mt-3 text-center" delay={0.9} useParentTrigger={true}>
                <p className="text-[12px] sm:text-[13px] text-gray-500 dark:text-gray-400 tracking-[0.015em] opacity-90">
                  Note: Each bar shows the lowest published end-to-end o200k p99 per implementation, so the best latency method can differ from the throughput winner.
                </p>
              </AnimatedSection>
            </div>
          </AnimatedSection>
          ...
          <AnimatedSection className="pt-8 sm:pt-10" delay={0.1}>
            <div className="border border-gray-200/80 dark:border-white/10 rounded-[24px] p-5 md:p-6 bg-white dark:bg-[#121212]/60 backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.08)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.2)]">
              <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-6 lg:gap-8 items-center">
                <div ref={videoFrameRef} className="relative w-full aspect-video rounded-[24px] overflow-hidden shadow-lg border border-white/20 dark:border-white/10 bg-black">
                  {isVideoPlaying ? (
                    <iframe
                      src="https://www.youtube.com/embed/G12OoKZ0bgY?autoplay=1&playsinline=1&rel=0&modestbranding=1"
                      title="Talifun Tokenizer launch video"
                    ...
                    <button
                      ...
                      aria-label="Play the launch video in fullscreen"
                      className="group relative w-full h-full text-left"
                    >
                      <img
                        src="https://i.ytimg.com/vi/G12OoKZ0bgY/maxresdefault.jpg"
                        alt="Talifun Tokenizer launch video thumbnail"
                        loading="lazy"
                        width="1280"
                        height="720"
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-white/18 border border-white/30 backdrop-blur-md text-white shadow-[0_12px_40px_rgba(0,0,0,0.35)] transition-transform duration-300 group-hover:scale-105 group-hover:bg-[#FF5100] group-hover:border-[#FF5100]">
                ...
                <div className="flex flex-col items-start lg:pl-4">
                  <h2 className="text-[32px] sm:text-[40px] font-bold text-[#171717] dark:text-white mb-4 sm:mb-6 leading-[1.1] tracking-[0.015em]">
                    From Benchmark Results to Real Pipeline Savings
                  </h2>
                  <p className="text-[16px] sm:text-[18px] text-gray-600 dark:text-gray-400 mb-8 leading-relaxed tracking-[0.015em] w-full pr-0 max-w-xl">
                    An overview of the benchmark methodology, the measured results, and the practical pipeline savings that come from faster tokenization.
                  </p>
                  <button
                    type="button"
                    onClick={() => playVideo(true)}
                  ...
                  >
                    Play the launch video fullscreen
